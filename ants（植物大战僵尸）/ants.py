...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while p is not beehive:
            if dis >= self.min_range and dis <= self.max_range:
                if len(p.bees) > 0:
...
# END Problem EC
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem EC
        if self.armor - amount <= 0 and self.true:
            bees_win()
        super().reduce_armor(amount)
        # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        return max(2 - distance * 0.2 - self.insects_shot * 0.05, 0)
        # END Problem Optional 5
